 ...
 * @LastEditors: happain
 * @LastEditTime: 2022-10-06 05:44:06
 * @Description:
-->
<script setup>
import scIconSelect from "@/components/scIconSelect";
const app = getCurrentInstance().appContext.config.globalProperties;
const emits = defineEmits(["upded"]);
// 初始化数据
const initData = (currentMenu, allMenu) => {
	// 深拷贝，防止修改父组件数据
	const tem = JSON.parse(JSON.stringify(currentMenu));
	const obj = {
		...tem,
		...tem.meta,
	};
	...
	// 赋值表单数据
	updMenuform.value = obj;
	console.log(updMenuform.value);
	//
	menuData.value = tranMenu(tem.id, allMenu);
...
const updMenu = async (form) => {
	...
	await form.validate(async (valid) => {
		...
		if (valid) {
			console.log(updMenuform.value);
			const result = await app.$API.menu.upd.post({
				...updMenuform.value,
			});
			if (result.code == 200) {
